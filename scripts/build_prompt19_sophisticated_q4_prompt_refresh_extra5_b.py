...
OUT_ROOT = SEED / "q4_prompt_refresh_experiment_extra5_b"
MOTION_OUT = MOTIONS / "q4_prompt_refresh_experiment_extra5_b" / "IIWA" / "IIWA"
...
ICONIC_OUT = OUT_ROOT / "motion_configs_prompt_v19_sophisticated_q4_prompt_refresh_extra5_b_iconic.json"
CONTEXTUAL_OUT = OUT_ROOT / "motion_configs_prompt_v19_sophisticated_q4_prompt_refresh_extra5_b_contextual.json"
HTML_OUT = OUT_ROOT / "prompt19_sophisticated_q4_prompt_refresh_extra5_b_compare_20260404_ko.html"
MANIFEST_OUT = OUT_ROOT / "manifest.json"

TARGETS = [
    {"dataset": "iconic", "idx": 53, "cue": "conditional_accept_pause_then_reach"},
    {"dataset": "iconic", "idx": 55, "cue": "resume_speaking_pull"},
    {"dataset": "iconic", "idx": 34, "cue": "cover_ears_too_loud"},
    {"dataset": "contextual", "idx": 40, "cue": "disengage_step_back_release"},
...
def generate() -> None:
    ...
    for spec in TARGETS:
        ...
        target_ids = {53, 55, 34} if spec["dataset"] == "iconic" else {40, 42}
        filtered = [r for r in rows if int(r["idx"]) in target_ids]
        if spec["dataset"] == "iconic":
            iconic_rows = filtered
        else:
            contextual_rows = filtered
...
def build_html() -> None:
    ...
    for spec in TARGETS:
        ...
        cards.append(
            f"""
            <article class="card">
              <div class="hdr"><div class="title">{html.escape(spec["dataset"])} · c{spec["idx"]} · {html.escape(spec["cue"])}</div></div>
...
<head>
  ...
  <title>Prompt19 Sophisticated Q4 Prompt Refresh Extra 5B</title>
  <style>
...
<body>
  <main class="wrap">
    <h1>Prompt 19 Sophisticated Q4 Prompt Refresh Extra 5B</h1>
    <p class="lead">새 prompt로 추가 생성한 5개 cue를 원본 sophisticated 결과와 비교합니다.</p>
