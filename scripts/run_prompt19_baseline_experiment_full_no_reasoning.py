...
BASE_OUT = SEED / "baseline_prompt19_full_no_reasoning"
MOTION_OUT = MOTIONS / "baseline_prompt19_full_no_reasoning"
...
HTML_OUT = BASE_OUT / "prompt19_baseline_experiment_full_20260408_ko.html"
...
def _all_targets() -> list[dict[str, Any]]:
    iconic = [{"dataset": "iconic", "idx": int(row["idx"]), "cue": row["cue"]} for row in _load_json(ICONIC_SRC)]
    contextual = [{"dataset": "contextual", "idx": int(row["idx"]), "cue": row["cue"]} for row in _load_json(CONTEXTUAL_SRC)]
    return iconic + contextual
...
def _latest_gif_from_bases(bases: list[Path], cue: str, cue_idx: int | None = None) -> Path | None:
    matches: list[Path] = []
    for base in bases:
        if not base.exists():
            continue
        safe = _safe_name(cue)
        if cue_idx is not None:
            matches.extend(base.rglob(f"*_{safe}_c{cue_idx}_tiled.gif"))
        matches.extend(base.rglob(f"*_{safe}_p*.gif"))
        matches.extend(base.rglob(f"*_{safe}_*.gif"))
...
def render_no_reasoning(robot: str = "IIWA", hz: int = 8) -> None:
    ...
    try:
        for _, config_path, out_dir in rows_by_file:
            ...
            for row in rows:
                safe_cue = _safe_name(row["cue"])
                if list(out_dir.glob(f"*_{safe_cue}_p*.gif")):
                    continue
                ...
                try:
                    ...
                    print(f"[render_no_reasoning] fail robot={robot} cue={row['cue']} idx={row['idx']}: {exc}", flush=True)
    finally:
        gen.close()
...
def build_html(robot: str = "IIWA") -> None:
    ...
    section_cards: dict[str, list[str]] = {"iconic": [], "contextual": []}
    targets_sorted = sorted(TARGETS, key=lambda x: (0 if x["dataset"] == "iconic" else 1, int(x["idx"])))
    for spec in targets_sorted:
        ...
        joint_dirs = [
            MOTION_OUT / "direct_joint" / dataset / robot,
            MOTIONS / "baseline_prompt19_direct_experiment_more20configs" / "direct_joint" / dataset / robot,
            MOTIONS / "baseline_prompt19_direct_experiment_extra10" / "direct_joint" / dataset / robot,
            MOTIONS / "baseline_prompt19_direct_experiment_nonoverlap10" / "direct_joint" / dataset / robot,
            MOTIONS / "baseline_prompt19_direct_experiment" / "direct_joint" / dataset / robot,
        ]
        xyz_dirs = [
            MOTION_OUT / "direct_xyz_theta" / dataset / robot,
            MOTIONS / "baseline_prompt19_direct_experiment_more20configs" / "direct_xyz_theta" / dataset / robot,
            MOTIONS / "baseline_prompt19_direct_experiment_extra10" / "direct_xyz_theta" / dataset / robot,
            MOTIONS / "baseline_prompt19_direct_experiment_nonoverlap10" / "direct_xyz_theta" / dataset / robot,
            MOTIONS / "baseline_prompt19_direct_experiment" / "direct_xyz_theta" / dataset / robot,
        ]

        ref_gif = _latest_single_gif(ref_dir, cue) or _latest_gif(ref_dir, cue, idx)
        no_reason_gif = _latest_gif(no_reason_dir, cue, idx)
        joint_gif = _latest_gif_from_bases(joint_dirs, cue, idx)
        xyz_gif = _latest_gif_from_bases(xyz_dirs, cue, idx)

        def media(path: Path | None, label: str) -> str:
            if path is None:
                return f'<div class="missing">{label}<br>No image</div>'
            return f'<img src="{path.resolve().as_uri()}" alt="{html.escape(label)}" loading="lazy">'

        section_cards[dataset].append(
            f"""
            <article class="card">
              <div class="hdr">
    ...
    sections_html = []
    for dataset, title in (("iconic", "Iconic"), ("contextual", "Contextual")):
        sections_html.append(
            f"""
            <section class="section">
              <div class="section-head">
                <h2>{title}</h2>
                <div class="section-meta">{len(section_cards[dataset])} cues</div>
              </div>
              <div class="grid">{''.join(section_cards[dataset])}</div>
            </section>
            """
        )
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Prompt19 Full Baseline</title>
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .section-head {{ display:flex; align-items:baseline; justify-content:space-between; gap:12px; margin:0 0 14px; padding-bottom:8px; border-bottom:1px solid var(--line); }}
...
<body>
  <main class="wrap">
    <h1>Prompt 19 Full Baseline</h1>
    <p class="lead">104개 전체 cue에 대해 Reference와 No Reasoning을 비교합니다. Direct q / Direct xyzθ는 현재 파일이 있는 경우에만 표시하고, 없으면 No image로 둡니다.</p>
