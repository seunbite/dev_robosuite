#!/usr/bin/env python3
"""Generate one locomotion config for a single cue using Gemini.

Adapted from dev_robosuite/adhoc/robotarm/config_gen_single.py for quadruped
locomotion configs (pose / movement / path schema).
"""
...
_DEFAULT_PROMPT = str(resolve_seed_prompt_txt("quadruped"))
_DEFAULT_SHOTS = str(resolve_seed_shots_json("quadruped"))
_DEFAULT_CONFIG = str(motion_configs_results_dir("quadruped") / "motion_configs_prompt_19_locomotion.json")
...
def _format_example_block(config: dict[str, Any]) -> str:
    """Format one shot config entry as a prompt example block with reasoning comments."""
    parts: list[str] = []
    ...
    exclude = {"reasoning", "state", "model", "time", "group", "cue_text"}
...
def _extract_reasoning_and_json(raw_text: str) -> tuple[str, dict[str, Any]]:
    """Extract reasoning comment lines and the best JSON object from model output."""
...
def _validate_reasoning(reasoning_text: str) -> list[str]:
    """Validate Q1/Q2/Q3 planning comment format."""
    errors: list[str] = []
...
def _validate_config(config: dict[str, Any]) -> list[str]:
    """Validate a generated locomotion config. Returns error strings (empty = valid)."""
    errors: list[str] = []
    ...
    if all(t == "pose" for t in step_types):
        errors.append("Must include at least one 'movement' or 'path' step")
    if len(movements) < 2:
        errors.append(f"Only {len(movements)} step(s); need at least 2")
    if len(movements) > 10:
        errors.append(f"{len(movements)} steps exceeds max of 10")

    valid_types = {"pose", "movement", "path"}
    valid_dirs = {"front", "back", "left", "right"}
    valid_heights = {"low", "mid", "high"}
    valid_hand_positions = {"up", "front", "right", "back", "left"}
    valid_shapes = {"line", "arc", "pose_to_pose"}
    ...
    for m in movements:
        ...
        if mtype == "pose":
            ...
            if d and d not in valid_dirs:
                errors.append(f"Invalid pose dir: {d}")
            for hk in ("front_height", "back_height"):
                h = pose.get(hk)
                if h and h not in valid_heights:
                    errors.append(f"Invalid {hk}: {h}")
            if pose.get("three_legs") and pose.get("three_leg_pose"):
                hp = pose["three_leg_pose"].get("hand_position")
                if hp and hp not in valid_hand_positions:
                    errors.append(f"Invalid hand_position: {hp}")
            ...
            speed = mv.get("speed")
            if isinstance(speed, (int, float)) and not (0.3 <= float(speed) <= 4.0):
                errors.append(f"Movement speed {speed} outside [0.3, 4.0]")
...
def _format_cue_catalog(yaml_path: str) -> str:
    """Load cues YAML and format it as a prompt-ready catalog string."""
...
def generate_motion_config(
    ...
    """Generate one locomotion config via Gemini and upsert into config_json."""
