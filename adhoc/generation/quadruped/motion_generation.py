...
def run(
    ...
    prompt = str(Path(prompt_file)) if prompt_file else str(resolve_seed_prompt_txt("quadruped"))
    shots = str(Path(shots_json)) if shots_json else str(resolve_seed_shots_json("quadruped"))
    out = Path(config_json) if config_json else motion_configs_results_dir("quadruped") / f"motion_configs_{cue_group}.json"
    ...
    failed = 0
    for cue_idx, (cue_name, _desc) in tqdm(indexed, desc="motion_generation[quadruped]"):
        ok = False
        last_err = ""
