...
MC_MANIP = os.path.join(PROJECT_ROOT, "data", "results", "motion_configs", "manipulator")
PROMPT_DIR = os.path.join(SEED_DIR, "prompt", "manipulator")
EXPECTED_CUES = 58
...
def _config_path(version: int) -> str:
    return os.path.join(MC_MANIP, f"motion_configs_prompt_v{version}.json")
...
def _expected_cue_map() -> dict[int, str]:
    cues_path = os.path.join(SEED_DIR, "yml", "cues.yml")
...
def generate(versions: list[int] = None, model: str = "gemini-2.5-flash", delay: float = 2.0):
    ...
    for v in versions:
        ...
        python_bin = sys.executable
        yaml_cues = os.path.join(SEED_DIR, "yml", "cues.yml")
        cmd = [
            python_bin,
            "adhoc/generation/config_generation.py",
            "--robot=manipulator",
            f"--yaml_path={yaml_cues}",
            "--cue_group=iconic",
...
def status(versions: list[int] = None):
    ...
    print(f"\n  Config files: {MC_MANIP}/motion_configs_prompt_v*.json")
...
def bestof(
    ...
    Args:
        ...
        output: Output JSON path (default: data/results/motion_configs/manipulator/motion_configs_bestof.json)
    ...
    if output is None:
        output = os.path.join(MC_MANIP, "motion_configs_bestof.json")
...
<div class="prompt-section">
  ...
  <div class="prompt-content open" style="font-family: inherit; white-space: normal; padding: 0; max-height: none;">
    <svg viewBox="0 0 1100 520" xmlns="http://www.w3.org/2000/svg" style="width: 100%; background: var(--surface2); border-radius: 0 0 8px 8px;">
      ...
      <text x="382" y="212" text-anchor="middle" fill="#3fb950" font-size="12" font-weight="600" font-family="-apple-system, sans-serif">config_generation.py</text>
...
def contextual_dashboard(
    ...
    config_path = os.path.join(MC_MANIP, f"motion_configs_prompt_v{version}_contextual.json")
