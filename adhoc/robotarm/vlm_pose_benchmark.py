...
def _load_configs(version: int) -> list[dict]:
    path = os.path.join(SEED_DIR, f"motion_configs_prompt_v{version}.json")
...
def main(
    ...
    if not test_cues:
        print(f"No rendered GIFs found for {robot}. Run meta_motion_generation.py first.")
        return
...
def trajectory(
    ...
    config_path = os.path.join(SEED_DIR, f"motion_configs_prompt_v{config_version}.json")
    configs = _load_configs(config_version)
    if not configs:
