...
Usage:
    ...
    python adhoc/spot/motion_generation_dog.py \
        --robot Go2 --cue body_bounce \
        --jsonl-path data/poses/quadruped/closest_dog_poses.jsonl

Note:
    Default pose database: data/poses/quadruped/closest_dog_poses.jsonl
    
    To generate poses, run:
    1. python adhoc/spot/stack_preset_dog.py --robot Go2
    2. python adhoc/spot/meta_find_closest_poses_dog.py
"""
...
class QuadrupedMotionGenerator:
    ...
    def _auto_detect_jsonl_path(self, robot_name: str) -> str:
        ...
        Returns:
            Path to JSONL file
        """
        # Default: use filtered closest poses
        closest_path = "data/poses/quadruped/closest_dog_poses.jsonl"
        if os.path.exists(closest_path):
            print(f"Using filtered dog poses: {closest_path}")
            return closest_path
        
        # Fallback: robot-specific generated poses
        robot_specific_path = f"data/poses/quadruped/{robot_name}/{robot_name}_dog_poses.jsonl"
        if os.path.exists(robot_specific_path):
            print(f"Using robot-specific poses: {robot_specific_path}")
            return robot_specific_path
        
        # Warning
        print(f"Warning: No pose database found for {robot_name}")
        print(f"Please run:")
        print(f"  1. python adhoc/spot/stack_preset_dog.py --robot {robot_name}")
        print(f"  2. python adhoc/spot/meta_find_closest_poses_dog.py")
        return closest_path
    ...
    def _load_cue_config(self, cue: str, config_path: str = "data/seed/motion_config_quadruped.json") -> Dict:
        """
    ...
    def execute_cue(
        ...
        config_path: str = "data/seed/motion_config_quadruped.json",
        hz: int = 4,
...
def generate(
    ...
    """
    Main function to generate quadruped (dog) robot motions.
    
    Args:
        robot: Robot name (e.g., "Go2", "SpotWithArm")
    ...
    Examples:
        ...
        python adhoc/spot/motion_generation_dog.py \
            --robot Go2 --cue body_bounce \
            --jsonl-path data/poses/quadruped/Go2/Go2_dog_poses.jsonl
    """
