...
def load_cues() -> Dict[str, Dict[str, str]]:
    with open(SEED_DIR / "cues.yml", "r", encoding="utf-8") as f:
...
def build_samples(
    *,
    ...
    if testset == "iconic":
        config_path = Path(config_json) if config_json else (SEED_DIR / f"motion_configs_prompt_v{prompt_version}.json")
        ...
        config_path = Path(config_json) if config_json else (SEED_DIR / f"motion_configs_prompt_v{prompt_version}_contextual.json")
        cfgs = _load_json(config_path)
        for cfg in cfgs:
            cue_idx = int(cfg["idx"])
...
def _build_middle_frame_with_trajectory(sample: Dict[str, Any], output_path: str, robot: str, hz: int) -> str:
    robotarm_dir = REPO_ROOT / "adhoc" / "robotarm"
    if str(robotarm_dir) not in sys.path:
        sys.path.insert(0, str(robotarm_dir))
    ...
    try:
        ...
        base = Image.fromarray(frames[mid_idx] if isinstance(frames[mid_idx], np.ndarray) else np.array(frames[mid_idx])).convert("RGB")
        draw = ImageDraw.Draw(base)
        pts = []
        for t in trajectory:
            p = _project_3d(t["pos"], cam_pos, cam_rot, fovy, base.size[0])
...
def prepare_test_media(
    ...
    *,
    ...
    robot: str = "IIWA",
    hz: int = 8,
    output_dir: str = "adhoc/test/eval_media_variants",
    force: bool = False,
) -> List[Dict[str, Any]]:
    canonical = {
        "first_frame": "first_frame_only",
        "first_frame_only": "first_frame_only",
        "gif": "gif",
        "mp4": "mp4",
        "alpha_stack_final": "alpha_stack_final",
        "alpha_stack_final_arrow": "alpha_stack_final_arrow",
        "middle_frame_only": "middle_frame_only",
        "middle_frame_trajectory": "middle_frame_trajectory",
    }.get(test_type)
    ...
    for sample in samples:
        ...
        if not out_png.exists() or force:
            if canonical == "first_frame_only":
                ...
                _save_png(image, str(out_png))
            elif canonical == "alpha_stack_final":
                image = _build_alpha_stack_image(gif_path)
                _save_png(image, str(out_png))
            elif canonical == "alpha_stack_final_arrow":
                _build_alpha_stack_with_arrows(sample, str(out_png), robot=robot, hz=hz)
            elif canonical == "middle_frame_trajectory":
                _build_middle_frame_with_trajectory(sample, str(out_png), robot=robot, hz=hz)
...
def prepare_normalized_eval_media(
    ...
    *,
    ...
    top_k: int = 1,
    output_dir: str = "adhoc/test/eval_media",
    force: bool = False,
) -> List[Dict[str, Any]]:
